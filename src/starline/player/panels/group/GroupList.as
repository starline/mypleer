package starline.player.panels.group {
	...
	public class GroupList extends Sprite {
		...
		public var searchPanel_mc:GroupMenu;
		...
		public function GroupList(_root:*, _parent:*) :void {
			this._root = _root;
			...
			searchPanel_mc = new GroupMenu(_root, this);
			searchPanel_mc.x = 8;	searchPanel_mc.y = 10;
			addChild(searchPanel_mc);
			
			close_btn.x = 273;	close_btn.y = 10;
			addChild(close_btn);
			close_btn.addEventListener(MouseEvent.MOUSE_UP, function(e:MouseEvent):void { _parent.groupList_mc.visible = false; } );
		}
		
		
		/**
		 * onList
		 * Формируем строки групп
		 */
		public function onList(arr:Array):void {
			...
			if (in_arr.length > 0) {
				for (var i:int = 0; i < in_arr.length; ++i) {
					var p:* = in_arr[i];
					var line:GroupLine = new GroupLine(this, i, p);
		...
		public function onListSortByKey(search_key:String = ''):void { // фильтровка аудиозаписей по слову
			...
			if (search_key == "") {
				...
				for (var i:uint = 0; i < lines_arr.length; i++) {
					var search_name:String = lines_arr[i]._name;
		...
		public function onDclick(n:int):void {
			...
			_parent.audioList_mc.gid = lines_arr[cur_dclick].gid;
			_parent.audioList_mc.menu_mc.changePage(2, false, true);
			_parent.audioList_mc.menu_mc.getGroupAudio(lines_arr[cur_dclick].gid);
