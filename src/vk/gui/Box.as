package vk.gui {
	...
	public class Box extends Sprite {
		...
		public static var STAGE_WIDTH:uint = 807;
		
		private static const BORDER_W:uint = 15;
		private static const TITLE_H:uint = 29;
		private static const BOTTOM_H:uint = 42;
		private static const FONT_SZ:Number = 12;
		private static const LINE_H:Number = FONT_SZ + 2;
		
		private static const MAX_CONTENT_HEIGHT:uint = 4 + LINE_H * 35;
		
		private static const CONTENT_X:uint = 30;
		private static const CONTENT_Y:uint = TITLE_H + CONTENT_X + 15;
		...
		public function Box( title:String, content:*, y:int, w_:uint, buttons:Array ):void {
			...
			// Фот подложки
			Utils.fillRect( this, -10000, -10000, 20000, 20000, 0, 0.05 );
			...
			if ( content is String ){
				txt = Utils.addText( CONTENT_X, CONTENT_Y, w - 2*CONTENT_X, FONT_SZ, content as String, 0, Utils.TXT_HTML | Utils.TXT_MULTILINE | Utils.TXT_AUTOSIZE );
				addChild( txt );
			...
			// Title
			addChild( Utils.addText( CONTENT_X, CONTENT_X, w - 10, 16, title, 0x000000, Utils.TXT_BOLD | Utils.TXT_AUTOSIZE ) );
			
			// Оконтовка окна
			Utils.fillRRect( this, -BORDER_W, -BORDER_W, w + 2 * BORDER_W, h + 2 * BORDER_W, 0x000000, 5, 0.1);
